...
export const newFight = async (
  ...
  try {
    ...
    const secondOpponent: ICharacter = await findOpponent(
      currentUser._id,
      firstOpponent._id,
      firstOpponent.level
    );
    const fight = await launchFight(firstOpponent, secondOpponent);
...
const launchFight = async (
  ...
  let fight = await FightDM.createFight(firstOpponent._id, secondOpponent._id);
  let turnCount = 0;
  let fightOver = false;
